...
def test2():
    ...
    img = Image.open('d:/dataset/irregular_mask/irregular_mask/disocclusion_img_mask/22972.png')
    # print('load ' + os.listdir('d:/dataset/irregular_mask/irregular_mask/disocclusion_img_mask'))
    npimg = np.array(img)
    print(np.unique(npimg))
    torchimg = torchvision.transforms.Compose([
        torchvision.transforms.ToTensor()
    ])(img)


    print(torch.unique(torchimg), torchimg.squeeze().numpy().dtype)
    plt.imshow(torchimg.squeeze().numpy(),cmap='gray')
    plt.figure()
    plt.imshow(npimg,cmap='gray')

    plt.figure()
    rgb =np.array(Image.open('D:\git\crawler\zigbang\\train\class\\13149440_3.jpg'))
    plt.imshow(rgb)
